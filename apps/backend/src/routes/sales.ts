...
router.put('/:id', authMiddleware, authorize('SUPERVISOR', 'JEFE_ZONAL', 'BACK_OFFICE', 'SUPERADMIN', 'GERENTE'), filterProtectedFields, async (req: any, res: any) => {
  try {
    ...
    if (data.cotizacion_monto !== undefined) data.cotizacion_monto = Number(data.cotizacion_monto);
    if (data.cotizacion_cuota !== undefined) data.cotizacion_cuota = Number(data.cotizacion_cuota);
    if (data.cotizacion_plazo !== undefined) data.cotizacion_plazo = Number(data.cotizacion_plazo);
    if (data.remesa_monto_original !== undefined) data.remesa_monto_original = Number(data.remesa_monto_original);
    if (data.remesa_monto_aprobado !== undefined) data.remesa_monto_aprobado = Number(data.remesa_monto_aprobado);
    if (data.compra_deuda_monto !== undefined) data.compra_deuda_monto = Number(data.compra_deuda_monto);
